...
public class ClientUpdateHandler
{
	...
	private readonly GameTitle _gameTitle = new();
	private readonly GameTitleHandler _gameTitleHandler;
	...
	public ClientUpdateHandler(ContainerControl containerControl, INetworkHandler networkHandler)
	{
		...
		_gameTitleHandler = new GameTitleHandler(_gameTitle);
	...
	private MenuStrip CreateMenu()
	{
		...
		fileMenu.Add(new MenuItemLeaf("Save", (s, e) => 
		{
			_gameTitleHandler.Backup();
			MessageBox.Show("Current title saved. Undo if need be ¯\\_(ツ)_/¯");
		}));
		fileMenu.Add(new MenuItemLeaf("Exit", (s, e) => Application.Exit()));

		var editMenu = new MenuComposite("Edit");
		editMenu.Add(new MenuItemLeaf("Change Title", (s, e) =>
		{
			var newTitle = Microsoft.VisualBasic.Interaction.InputBox("Prompt", "Title", "Default", 0, 0);
			_gameTitle.SetTitle(newTitle);
		}));
		editMenu.Add(new MenuItemLeaf("Undo", (s, e) => _gameTitleHandler.Undo()));
